...
sub permutate {
	...
	for (my $i = 0; $i < @_; ++$i) {
		my @copy = @_;
		my $x = splice @copy, $i, 1;
		foreach my $p (permutate(@copy)) {
			push @result, [$x, @$p];
		}
	}
	return @result;
...
my @perms = permutate(@wires);
print join("\n", map { join(",", @$_) } @perms), "\n";

sub translate {
	my($p, $digit) = @_;
	my @result = ();
	for (my $j = 0; $j < length($digit); ++$j) {
		my $letter = substr($digit, $j, 1);
		for (my $i = 0; $i < @$p; ++$i) {
			if ($p->[$i] eq $letter) {
				push @result, $wires[$i];
			}
...
sub arrayeq {
	my($p, $q) = @_;
	return join(",", sort @$p) eq join(",", sort @$q);	
}
...
sub makes_sense {
	...
	foreach my $digit (@$c) {
		for (my $k = 0; $k < @wiring; ++$k) {
			my $w = $wiring[$k];
			if (arrayeq([translate($p, $digit)], $w)) {
				$found[$k]++;
				last;
			}
		}
	}
	return join("", @found) eq '1111111111';
}

print join(",", translate([qw(g f e d c b a)], "gbfced")), "\n";
print arrayeq([1, 2], [3, 4]), "\n";
print arrayeq([1, 2], [2, 1]), "\n";
...
while (my $line = ($input =~ /^([^|]*?)\s*\|\s*(.*?)\s*$/mg)) {
	...
	foreach my $p (@perms) {
		if (makes_sense($p, \@c)) {
			print join(".", @$p), "\n";
			foreach my $value (@d) {
				my @translated = translate($p, $value);
				for (my $k = 0; $k < @wiring; ++$k) {
					my $w = $wiring[$k];
					if (arrayeq(\@translated, $w)) {
						$number .= $k;
						last;
					}
				}
