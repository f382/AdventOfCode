#!/usr/bin/perl -w
use strict;

my $input1 = <<EOF
[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]
[[[5,[2,8]],4],[5,[[9,9],0]]]
[6,[[[6,2],[5,6]],[[7,6],[4,7]]]]
[[[6,[0,7]],[0,9]],[4,[9,[9,0]]]]
[[[7,[6,4]],[3,[1,3]]],[[[5,5],1],9]]
[[6,[[7,3],[3,2]]],[[[3,8],[5,7]],4]]
[[[[5,4],[7,7]],8],[[8,3],8]]
[[9,3],[[9,9],[6,[4,9]]]]
[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]
[[[[5,2],5],[8,[3,7]]],[[5,[7,5]],[4,4]]]
EOF
;

my $input2 = <<EOF
[6,[[5,[7,7]],[[8,2],2]]]
[[8,[0,0]],[[[1,4],[2,0]],[[2,3],[8,2]]]]
[[[7,[6,1]],[9,[7,9]]],[[6,6],2]]
[[5,[2,0]],[[[9,4],[6,8]],[3,9]]]
[[[3,[0,3]],[5,[9,8]]],[[5,[8,1]],[1,2]]]
[[[5,[8,5]],[[6,3],3]],[[1,[0,9]],[[3,0],[7,3]]]]
[[[[1,2],0],[8,[6,6]]],[6,[7,5]]]
[[[[0,9],[5,3]],[[9,7],8]],[9,[[1,9],1]]]
[[4,[6,[0,8]]],[[2,9],1]]
[[[5,1],[6,[9,5]]],8]
[[[4,[7,0]],1],[3,3]]
[2,[[3,4],6]]
[[[[0,5],[7,1]],[[7,0],[1,7]]],2]
[[3,[7,8]],[[1,0],[[1,7],6]]]
[[[7,[6,3]],1],[4,[1,[6,8]]]]
[[[[5,5],[3,5]],[5,[2,6]]],[3,[1,[3,2]]]]
[[8,2],9]
[[8,[[1,9],2]],[[8,[8,4]],3]]
[[[[8,7],9],5],[[[0,6],5],[6,5]]]
[[2,6],[0,[[9,8],6]]]
[[[[7,9],0],[[1,5],9]],[[[2,2],1],[3,1]]]
[[8,[1,[9,1]]],[0,[0,[2,4]]]]
[[[[0,2],3],[[9,4],9]],[[3,2],[[5,7],[4,8]]]]
[[[[8,0],3],[9,[5,9]]],[4,3]]
[2,[7,[[3,3],[7,9]]]]
[3,[[[6,4],0],5]]
[[1,[[9,9],9]],[[[0,3],[0,6]],0]]
[[7,[[2,3],4]],[6,[9,9]]]
[3,5]
[[[[2,3],2],[1,[9,9]]],[[8,2],[1,[2,0]]]]
[[[[7,2],[3,6]],[[9,7],[8,9]]],[[[5,3],[5,1]],6]]
[8,[[7,0],[[8,2],5]]]
[[[1,[1,0]],[[3,7],[4,5]]],[[[6,5],2],1]]
[8,[[5,0],3]]
[[[4,1],9],[1,5]]
[[9,[5,[3,7]]],[[0,4],4]]
[[[[9,6],4],[8,[7,5]]],[[6,9],5]]
[[[7,[2,5]],0],[8,[0,7]]]
[8,[[4,[1,8]],[[8,9],3]]]
[[[[3,7],[7,3]],[[5,3],1]],[[[6,7],[2,7]],[[2,6],[9,0]]]]
[[[9,[5,4]],[[1,7],2]],[[1,0],[[2,6],0]]]
[[[[3,2],[5,3]],[9,[2,0]]],[[7,6],[[8,2],[0,7]]]]
[[[4,0],0],[9,0]]
[[[[9,4],[0,9]],[2,3]],8]
[[2,[8,[6,7]]],[[[6,1],[5,1]],[3,[1,1]]]]
[[6,[[8,2],7]],5]
[[[[7,3],[9,7]],5],[[[0,9],4],[3,[0,3]]]]
[[2,[6,7]],[[[9,0],[6,7]],4]]
[[[[3,1],5],[7,[5,1]]],[[[8,4],9],[[2,7],[4,6]]]]
[[8,8],[[[1,4],[7,3]],[[9,6],5]]]
[[[[3,0],8],[[5,1],[7,8]]],[[[5,0],[2,2]],[[9,0],[0,7]]]]
[[1,1],7]
[[4,3],[[9,[7,3]],[2,3]]]
[[[5,[3,5]],3],7]
[[[5,[3,2]],1],[[[3,2],8],[8,5]]]
[[7,[6,5]],[[[9,8],7],6]]
[[[7,9],0],[3,4]]
[[[[5,6],[7,4]],[4,[1,7]]],[[[8,2],7],[6,[4,5]]]]
[[[[8,5],1],[[7,0],[9,7]]],[[[5,2],3],2]]
[[[[7,3],9],7],[[[8,1],9],3]]
[[[6,5],[4,[6,1]]],7]
[[[[4,0],1],[4,4]],[[1,7],[3,0]]]
[9,[[2,[9,0]],6]]
[[[[0,0],9],2],[9,3]]
[5,[5,[[6,6],[2,6]]]]
[[[[8,2],[8,9]],[[8,4],[8,0]]],[[[9,5],6],4]]
[[[3,[6,1]],[3,[6,9]]],[3,4]]
[[[7,[6,3]],[[6,7],1]],[[1,1],2]]
[[[[1,2],[7,1]],[[7,7],4]],3]
[[[[7,2],4],[[4,7],[2,4]]],[1,[[6,2],4]]]
[4,[[0,4],5]]
[[[4,6],[0,[8,2]]],[[4,[8,7]],[[7,9],0]]]
[[[9,2],[5,2]],7]
[[9,[2,2]],4]
[[[6,2],[[4,3],[3,9]]],[[4,1],[[9,4],4]]]
[[[7,[4,2]],4],[[8,[9,3]],[[9,0],[5,4]]]]
[[[[4,1],[4,6]],[2,[4,5]]],[[1,[1,5]],9]]
[[[[5,2],8],6],[1,[[8,9],4]]]
[[[6,[4,2]],[6,[5,5]]],[0,3]]
[[7,[5,[7,0]]],[[[7,2],0],[[7,3],8]]]
[[3,[[2,3],[2,8]]],[5,7]]
[[[[8,0],[6,4]],[[3,7],3]],[[[7,3],[9,8]],[[8,0],8]]]
[[2,[5,2]],[[[0,9],[4,1]],[[8,8],4]]]
[[[[4,1],[6,5]],[[2,7],[5,8]]],[[[7,1],[2,3]],2]]
[[3,5],2]
[[9,2],[[6,[1,9]],[9,5]]]
[[4,[3,8]],[[4,[0,3]],[1,1]]]
[[[6,[0,7]],[[0,4],[6,1]]],[[8,5],[[5,2],[7,2]]]]
[[[[8,8],[6,3]],[[0,2],[6,5]]],[[[7,6],[5,4]],[4,[7,1]]]]
[[[[7,6],[5,8]],[6,1]],[0,[[0,8],9]]]
[[[6,[2,2]],[5,2]],6]
[[[7,9],[[1,5],8]],2]
[[[3,3],[5,6]],[[3,[3,6]],[2,[5,9]]]]
[[[[0,8],[0,1]],[[8,6],4]],[[0,[1,3]],4]]
[[6,[4,4]],[4,[0,4]]]
[[[3,4],[4,1]],8]
[[0,[[4,6],[5,1]]],[[0,[4,3]],[6,9]]]
[[[0,7],2],[[4,9],[1,6]]]
[[1,0],[[[4,9],9],[[5,4],9]]]
[8,[[[1,4],[5,5]],[1,[0,9]]]]
EOF
;

#################
my $input = $input2;

sub parse {
	my $tt = shift;
	if ($$tt =~ s/^\[//) {
		# print "[\n";
		my $left = parse($tt);
		$$tt =~ s/^,//;
		# print ",\n";
		my $right = parse($tt);
		$$tt =~ s/^\]//;
		# print "]\n";
		return [$left, $right];
	} elsif ($$tt =~ s/^(\d+)//) {
		# print "$1\n";
		return $1;
	}
	die "$$tt\n";
}

sub debug {
	my $x = shift;
	if (ref $x) {
		return "[" . join(",", map { debug($_) } @$x) . "]";
	}
	return $x;
}

sub explode {
	my($x, $d, $c, $cc) = @_;
	if (!ref($x)) {
		++$$cc;
		return;
	}
	if ($d >= 4 && !ref($x->[0]) && !ref($x->[1])) {
		return ($c, @$x);
	}
	my @e = explode($x->[0], $d + 1, $$cc, $cc);
	return @e if @e;
	@e = explode($x->[1], $d + 1, $$cc, $cc);
	return @e;
}

sub do_explode {
	my($xx, $d, $c, $cc, $k, $l, $r) = @_;
	# print "$d, $c, $$cc? $k, $l, $r? $$xx\n";
	my $x = $$xx;
	if (!ref($x)) {
		++$$cc;
		if ($c == $k - 1) {
			$$xx += $l;
		}
		if ($c == $k + 1) {
			$$xx += $r;
		}
		return;
	}
	if ($c == $k && !ref($x->[0])) {
		$$xx = 0;
		++$$cc;
		return;
	}
	do_explode(\$x->[0], $d + 1, $$cc, $cc, $k, $l, $r);
	do_explode(\$x->[1], $d + 1, $$cc, $cc, $k, $l, $r);
}

sub hit {
	my($xx) = @_;
	my $x = $$xx;
	if (!ref($x)) {
		if ($x >= 10) {
			my $l = int($x / 2);
			my $r = $x - $l;
			$$xx = [$l, $r];
			return $x;
		}
		return;
	}
	my $s = hit(\$x->[0]);
	return $s if defined($s);
	$s = hit(\$x->[1]);
	return $s;
}

sub magn {
	my($x) = @_;
	if (!ref($x)) {
		return $x;
	}
	return 3 * magn($x->[0]) + 2 * magn($x->[1]);
}

sub clone {
	my($x) = @_;
	if (!ref($x)) {
		return $x;
	}
	return [ map { clone($_) } @$x ];
}

my @n = ();

while ($input =~ /^(.*)$/mg) {
	my $t = $1;
	chomp($t);
	my $x = parse(\$t);
	print "\n", debug($x), "\n";
	push @n, $x;
}

my $max;

for (my $i = 0; $i < @n; ++$i) {
	for (my $j = 0; $j < @n; ++$j) {
		next if $i == $j;
		my $n = [clone($n[$i]), clone($n[$j])];
		print "\n$i, $j: ", debug($n), "\n";
		while (1) {
			my $c = 0;
			my @e = explode($n, 0, $c, \$c);
			if (@e) {
				my($k, $l, $r) = @e;
				print "explode $k: $l, $r\n";
				$c = 0;
				do_explode(\$n, 0, $c, \$c, @e);
				print debug($n), "\n";
				next;
			}
			my $s = hit(\$n);
			if ($s) {
				print "split $s\n";
				print debug($n), "\n";
			} else {
				last;
			}
		}
		my $m = magn($n);
		print "mangitude: $m\n";
		$max = $m if !defined($max) || $max < $m;
	}
}

print "\nmax: $max\n";
